...
var Drone = function (options) {
  ...
  var uuid = (typeof options == 'string' || typeof options == 'String' ? options : undefined);
  options = options || {};
...
Drone.prototype.disconnect = function (callback) {
  if (this.connected) {
    ...
    this.peripheral.disconnect(function (error) {
      debug('Disconnected from drone: ' + this.name);
      this.connected = false;
      callback(error);
    }.bind(this));
  } else {
...
function down(options) {
  if (this.status.flying) {
    ...
    this.logger("RollingSpider#down when it's not in the air isn't going to do anything");
  }
